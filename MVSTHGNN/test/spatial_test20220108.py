...
def gen_batch_G(X):
    ...
    """
    batch_G = []
    for _idx in x.size(0):
        cur_x = x[_idx]
        # gen G: n * n
        cur_G = gen_G(cur_x)
        batch_G.append(cur_G)
...
def train_it():
    ...
    for epoch in range(args.epochs):
        ...
        for idx, (data_x, data_y) in enumerate(train_dataloader, 0):
            ...
            lbl = data_y.long().to(device)

            # bz * n_frame * 3_view * 18 * xyp ---->> bz * n * c
            data = data.view(data.size(0), -1, 3)
            ...
            # cnt_cor = (accuracy_score(data_y, train_pred)).sum()
        ...
        for idx, (data_x, data_y) in enumerate(valid_dataloader, 0):
            ...
            acc = torch.mean((valid_pred == data_y).float())
            ...
            f_vilad_score = f1_score(data_y, valid_pred, average='weighted')
