...
def train_it():
    ...
    for epoch in range(args.epochs):
        ...
        for idx, (data_x, data_y) in enumerate(train_dataloader, 0):
            ...
            # Gs = torch.repeat(G.unsqueeze(0), (data.size(0), 1, 1))

            optimizer.zero_grad()

            # compute output
            outputs = net(data, G)  #torch.float32
        ...
        print("epoch={}/{},{}/{} of train, loss={}, mean training acc is {:.3f}%, precision is {:.3f}%, recall is {:.3f}, f1 is {:.3f}".format(
                    epoch, args.epochs, idx, len(train_dataloader), loss.item(), acc*100, pre_score * 100, rec_score, f_score))
        ...
        lbl_result = np.array(lbl_list)
        valid_acc = np.mean(valid_pred_result == lbl_result)
        pre_valid_score = precision_score(lbl_result, valid_pred_result, average='weighted')
        rec_valid_score = recall_score(lbl_result, valid_pred_result, average='weighted')
        f_valid_score = f1_score(lbl_result, valid_pred_result, average='weighted')

        valid_epoch_loss.append(loss.item())
        valid_loss.append(loss.item())

        print("epoch={}/{},{}/{} of valid, loss={}, mean validing acc is {:.3f}%, precision is {:.3f}%, recall is {:.3f}, f1 is {:.3f}".format(
            epoch, args.epochs, idx, len(valid_dataloader), loss.item(), valid_acc * 100, pre_valid_score * 100, rec_valid_score, f_valid_score))
