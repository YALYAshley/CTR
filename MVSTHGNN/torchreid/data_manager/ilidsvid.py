...
class iLIDSVID(object):
    ...
    def __init__(self, root='data', split_id=0, verbose=True, **kwargs):
        ...
        self._prepare_split() # split the training data and testing data for 10 times (cross validation), saved in json file
        splits = read_json(self.split_path) # size: 10*[train_idx_list,test_idex_list]
        if split_id >= len(splits):
            raise ValueError("split_id exceeds range, received {}, but expected between 0 and {}".format(split_id, len(splits)-1))
        split = splits[split_id] # get the split of training and testing data index
        train_dirs, test_dirs = split['train'], split['test']
        print("# train identites: {}, # test identites {}".format(len(train_dirs), len(test_dirs)))

        # get the train and test data (just tha name of each images)
        train, num_train_tracklets, num_train_pids, num_imgs_train = \
          self._process_data(train_dirs, cam1=True, cam2=True) # train data: enable both the camera situation, size:300
        query, num_query_tracklets, num_query_pids, num_imgs_query = \
          self._process_data(test_dirs, cam1=True, cam2=False) # query data : only enable camera 1, size:150
        gallery, num_gallery_tracklets, num_gallery_pids, num_imgs_gallery = \
          self._process_data(test_dirs, cam1=False, cam2=True) # gallery data : only enable camera 2, size:150

        num_imgs_per_tracklet = num_imgs_train + num_imgs_query + num_imgs_gallery
        min_num = np.min(num_imgs_per_tracklet) # the min seq_len in tha data
        ...
        # num_pid = number of person id, also the number of classes
    ...
    def _process_data(self, dirnames, cam1=True, cam2=True):
        tracklets = [] # save all of the img names of each sequence, size:(the num of sequences,seq_len(the num of images))
        num_imgs_per_tracklet = [] # save the img num of each sequence
        dirname2pid = {dirname:i for i, dirname in enumerate(dirnames)} # is a dict {img_name:index}
        
        for dirname in dirnames:
            if cam1:
                person_dir = osp.join(self.cam_1_path, dirname)
                img_names = sorted(glob.glob(osp.join(person_dir, '*.png'))) # get the names of each image in a sequence
                assert len(img_names) > 0
                img_names = tuple(img_names)
                pid = dirname2pid[dirname] # the index of a single image
        ...
        num_tracklets = len(tracklets) # 300
        num_pids = len(dirnames) # 150
